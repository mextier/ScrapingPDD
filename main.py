    ...
    for url in bileti_urls:
        ...
        try:
            ...
            try:
                ...
                try:
                    makedirs(join(dirname, const.bilet_dir_prefix +
                                  const.bilet_dir_format.format(bn)))
                    qs = bsObj.findAll('p', {'align': 'center'}, {
                                       'class': 'стиль10'})
                    bnq = 0
                    for q in qs:
                        print('[{0}] {1:.0f}%'.format(
                            '#' * int((const.progress_size * bnq / len(qs))), 100 * bnq / len(qs)), end='\r')
                        questionTitle = ""
                        questionImageUrl = ""
                        questionOptions = []
                        questionAnswer = 0
                        ...
                        if q.get_text().startswith(const.bilet_ptext):
                            ...
                            questionTitle = qparent.find('span', {'class': 'стиль2'}).get_text(
                            ).replace("\r", "").replace("\n", "").lstrip().rstrip()
                            q = qparent.find('img')
                            if q is not None:
                            ...
                            if questionImageUrl:
                                imageURL = urllib.parse.urljoin(
                                    const.pdd_bilet_url, questionImageUrl)
                    ...
                    print("Ошибка: Билет:" + str(bn) +
                          " Вопрос:" + str(bnq) + ":" + str(e))
